...
int recv_ack(Word seq_num_check, Word ack_num_check, Word SYN_check, Word ACK_check, Word FIN_check){
	...
	if (request_len < 0){
	    ...
	    if(SYN == 1 && ACK == 1){
	    	if(seq_num_check+1 != ack_num)
	    		return 0;
	    }else if(ACK == 1){
	    	if((ack_num_check != seq_num) && (seq_num_check+1 != ack_num)){
	    		printf("ACK error check\n");
	          	return 0;
	        }
	    }else if(FIN == 1){
	    	if(ack_num_check != seq_num && ack_num != 0){
	    		printf("fin error seq_num %d ack_num %d\n", seq_num, ack_num);
	    		return 0;
	    	}printf("fin error out seq_num %d ack_num %d\n", seq_num, ack_num);
	    }
