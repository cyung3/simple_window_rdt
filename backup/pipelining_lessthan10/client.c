...
void loop(){
	...
	switch(status){
	    ...
	    case Connected:
	     	send_data();     	
	     	save_before_close_seq = (cur_ack_num-1)%MAX_SEQUENCE_NUMBER;
	     	status = Closing;
	    break;
	    ...
	    case Closing:
    		...
    		if((ACK == 1) && (ack_num == (cur_seq_num+1)%MAX_SEQUENCE_NUMBER) && (seq_num == save_before_close_seq) && (FIN == 0) && (SYN == 0)){
	    		//saveack = cur_ack_num;
	    		save_seq = seq_num;
	    		save_ack = ack_num;
	    		status = Closed;
	    ...
	    case Closed:
			...
			if(FIN == 1 && seq_num == save_seq && ack_num == 0 && SYN==0 && ACK==0){
				cur_ack_num = (seq_num+1)%MAX_SEQUENCE_NUMBER;
				send_packet(save_ack, (seq_num+1)%MAX_SEQUENCE_NUMBER, 0, 1, 0, 12, NULL);
...
//needs to be updated to pipeline
void send_data(){
	//printf("Sending data\n");
	int bytesRead = 0;
	...
	if(fp != NULL){
		...
		//wrong!!!!!!!!!
		//first segment of each round must have an ack num
		//srver uses first ack_num to 
		// bytesRead = fread(data_buf, 1, MAX_DATA_SIZE, fp);
		// if(bytesRead < 0){
		// 	perror("error reading file\n");
		// }else{
		// 	memcpy((pipeline+(MAX_UDP_SIZE*curSegment)),data_buf, (bytesRead+12));
		// 	send_packet(cur_seq_num, cur_ack_num, 0, 1, 0, (bytesRead+12), data_buf);
		// 	cur_seq_num += bytesRead;
		// 	curSegment++;
		// 	//reset data_buf?
		// }
		/*
				...
				for(;curSegment < 10; curSegment++){
					...
					if(bytesRead < 0){
						...
						if(curSegment == 0){
							...
							cur_seq_num = (cur_seq_num + bytesRead)%MAX_SEQUENCE_NUMBER;
						}else{
				...
				for(;curSegment < numSegmentsInPipe; curSegment++){
					...
					if(ACK == 1){
						if(cur_seq_num == ack_num && cur_ack_num == seq_num){
								//why delete, just count down
								//memset(datagram, 0, MAX_UDP_SIZE);
								//update here?
								printf("updating\n");
								cur_seq_num = ack_num;
								cur_ack_num = (seq_num + 1)%MAX_SEQUENCE_NUMBER;
