  ...
  switch(status){
    ...
    case Waiting: 
      ...
      if((SYN==0) && (ACK==1) && (FIN==0) && (request_len > 12)){
        ...
        cur_ack_num = (seq_num + datalength) % MAX_SEQUENCE_NUMBER;
    ...
    case Connected:
      ...
      if(ACK == 1){
        ...
        cur_seq_num = (cur_seq_num + 1) % MAX_SEQUENCE_NUMBER;
      }
      else if((SYN==0) && (ACK==0) && (FIN==0) && (request_len >= 12)){
        ...
        //idk about this
        if((cur_ack_num+datalength != seq_num) && (ack_num != 0)){
          return;
        }
        ...
        build_ack(cur_seq_num, (cur_ack_num+datalength)%MAX_SEQUENCE_NUMBER, 0, 1, 0, response_buf);
        ...
        numSegmentsInPipe++;
        cur_ack_num = (cur_ack_num + datalength) % MAX_SEQUENCE_NUMBER;
